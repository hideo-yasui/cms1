;(function($, undefined){
	...
	var _table = [
		'<div class="styleName">',
		'<table class="tableStyleName">',
		'<thead>',
		'</thead>',
		'<tbody>',
		'</tbody>',
		'</table>',
		'<div class="list--user__pagenate">',
		'<ul class="list--user__pagenate_memu"></ul>',
		'</div>',
		'</div>'
	].join('');
	var _edit = '<a href="javascript:void(0);" title="編集" onclick="" class="btn btn--basic btn--small icons" style="background-color:#3A6;" accesskey="rowedit"><span class="icon edit"></span></a>';
	var _copy = '<a href="javascript:void(0);" title="コピー追加" onclick="" class="btn btn--basic btn--small icons" style="background-color:#36A;" accesskey="rowcopy"><span class="icon copy"></span></a>';
	var _delete = '<a href="javascript:void(0);" title="削除" onclick="" class="btn btn--basic btn--small icons" style="background-color:#F66;" accesskey="rowdelete"><span class="icon del"></span></a>';
	var _table = null;
	// definition
	function listTable(element, options) {
		...
		$("table", this._table).prop("class", options.tableStyleName);
	...
	listTable.prototype.refresh = function(data) {
		...
		var _html = this.__getHeaderHtml()+this.__getBodyHtml();
		_table.html(_html);
		this.__eventSetting();
	};
			...
			if(type=="checkbox") {
				...
				if(!util.isEmpty(this.options.header[key]["sort"])){
					switch(this.currentSort[this.options.header[key]["sort"]]){
						case "desc" :
							header += '　<span class="ui-icon ui-icon-triangle-1-s"></span></a>';
							break;
						case "asc" :
							header += '　<span class="ui-icon ui-icon-triangle-1-n"></span></a>';
							break;
						default:
							header += '　<span class="ui-icon ui-icon-triangle-2-n-s"></span></a>';
					}
				}
				header += '</th>';
			}
	...
	listTable.prototype.__eventSetting = function() {
		_table.find('a').unbind("click");
		_table.find('a.btn[accesskey*=row]').unbind("click");
		...
		if(_table.find('a.btn').length){
			_table.find('a.btn').on("click", function(event){
				event.preventDefault();
				var idx = $("#__index__", $(this).parent().parent()).val();
				if (idx >= 0) {
